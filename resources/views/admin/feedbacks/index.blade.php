...
@section('page_title', 'Quản lý Phản Hồi Khách Hàng')
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-gray-700 text-sm">
            ...
            <tr class="border-b hover:bg-gray-50/50 transition">
                ...
                <td class="p-4 font-semibold text-gray-900">{{ $fb->name }}</td>
                ...
                <td class="p-4 text-center">
                    <div class="flex justify-center items-center gap-3">
                        <a href="{{ route('feedbacks.show', $fb->id) }}" class="text-blue-600 hover:text-blue-800 font-bold transition">Xem</a>
                        <form action="{{ route('feedbacks.destroy', $fb->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa phản hồi này?');" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-800 font-bold transition">Xóa</button>
                        </form>
                    </div>
                </td>
            </tr>
