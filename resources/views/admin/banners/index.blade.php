    ...
    <div class="p-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <button type="submit" 
                        form="bulkDeleteBannerForm" 
                        id="bulkDeleteBtn" 
                        onclick="return confirmBulkDelete(event)" 
                        disabled 
                        class="bg-red-500 text-white px-5 py-2.5 rounded-xl opacity-50 cursor-not-allowed transition font-medium text-sm shadow-md flex items-center gap-1.5">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                    <span>Xóa mục đã chọn (0)</span>
                </button>

                <a href="{{ route('banners.create') }}" class="bg-[#e8634a] text-white px-6 py-2.5 rounded-xl hover:bg-[#d5523b] transition font-medium text-sm shadow-md flex items-center gap-2">
        ...
        <form id="bulkDeleteBannerForm" action="{{ route('banners.bulk_delete') }}" method="POST">
            @csrf
                        ...
                        @forelse($banners as $banner)
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="ids[]" value="{{ $banner->banner_id }}" class="row-checkbox w-4 h-4 accent-[#e8634a] rounded cursor-pointer">
                                </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.confirmBulkDelete = function (e) {
                ...
                return confirm(`Bạn có chắc chắn muốn xóa ${checkedCount} Banner đã chọn?`);
            };
        });
    </script>
@endsection
