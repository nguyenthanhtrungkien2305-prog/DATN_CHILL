...
@section('title', 'Sửa Người Dùng - Chill Chill Admin')
        ...
        <form action="{{ route('users.update', $user->user_id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" placeholder="Ví dụ: 0987654321" 
                           class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="Ví dụ: an.nguyen@example.com" 
                           class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Mật khẩu mới (Tùy chọn)</label>
                    <input type="password" name="password" placeholder="Nhập để đổi, để trống nếu giữ nguyên" 
                           class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="point" value="{{ old('point', $user->point) }}" min="0" required 
                           class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="address" rows="3" placeholder="Ví dụ: 123 Đường Nguyễn Huệ, Quận 1, TP. HCM" 
                          class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[#e8634a]">{{ old('address', $user->address) }}</textarea>
            </div>
            ...
            <div class="pt-6 flex gap-4 justify-end">
                ...
                <button type="submit" class="px-6 py-3 bg-[#e8634a] text-white rounded-lg hover:bg-[#d5523b] font-medium transition shadow-sm">Cập nhật</button>
            </div>
        </form>
    </div>
</div>
