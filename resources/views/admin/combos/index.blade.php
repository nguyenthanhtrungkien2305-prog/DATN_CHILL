...
@section('content')
    ...
    <div class="p-4 md:p-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            <p class="text-gray-500 text-xs sm:text-sm">Quản lý các gói Combo tiết kiệm, kết hợp nhiều sản phẩm với giá ưu đãi.</p>
            <div class="flex items-center gap-2 shrink-0 flex-wrap">
                ...
                <a href="{{ route('combos.create') }}" class="bg-[#e8634a] text-white px-4 py-2 rounded-xl hover:bg-[#d5523b] transition font-bold text-xs shadow-sm whitespace-nowrap">
                    + Thêm gói Combo mới
                </a>
            </div>
        </div>
        ...
        <div class="bg-white p-4 rounded-t-2xl border-t border-x border-gray-100 flex items-center justify-between">
            <form action="{{ route('combos.index') }}" method="GET" class="w-full flex flex-col sm:flex-row gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm tên combo..." class="flex-1 px-3 py-2 border rounded-xl focus:outline-none focus:border-[#e8634a] text-xs sm:text-sm">
                <select name="status" class="px-3 py-2 border rounded-xl text-xs sm:text-sm bg-white">
                ...
                <button type="submit" class="bg-gray-800 text-white px-4 py-2 rounded-xl hover:bg-gray-700 transition text-xs sm:text-sm font-bold">Lọc</button>
            </form>
        </div>
