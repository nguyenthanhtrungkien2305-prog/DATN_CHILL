...
@section('title', 'Chỉnh sửa Combo Sản phẩm - Chill Chill Admin')

@section('content')
    <header class="h-16 bg-white shadow-sm flex items-center justify-between px-8 shrink-0">
        <h2 class="text-xl font-semibold text-gray-800">Chỉnh Sửa Gói Combo Sản Phẩm</h2>
    ...
    <div class="p-8 max-w-5xl mx-auto">
        ...
        <form action="{{ route('combos.update', $combo->combo_id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $combo->price) }}" required min="0" step="1000"
                ...
                <div>
                    ...
                    <input type="number" name="original_price" id="original_price_input" value="{{ old('original_price', $combo->original_price) }}" min="0" step="1000"
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Hình ảnh Combo mới</label>
                    ...
                    @if($combo->image_url)
                        <div class="mt-2 flex items-center gap-3">
                            <img src="{{ asset($combo->image_url) }}" alt="Preview" class="w-12 h-12 object-cover rounded-lg border">
                            <span class="text-xs text-gray-400">Ảnh hiện tại</span>
                        </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="image_url" value="{{ old('image_url', $combo->image_url) }}" 
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" placeholder="Mô tả hấp dẫn về combo..." 
                              class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:border-[#e8634a] text-sm">{{ old('description', $combo->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:border-[#e8634a] text-sm bg-white">
                        <option value="1" {{ old('status', $combo->status) == '1' ? 'selected' : '' }}>Hiển thị ngay trên trang chủ</option>
                        <option value="0" {{ old('status', $combo->status) == '0' ? 'selected' : '' }}>Tạm ẩn</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Chỉnh sửa danh sách sản phẩm và số lượng tương ứng trong gói combo này.</p>
                    </div>
                ...
                <div id="combo-items-container" class="space-y-3">
                    @php
                        $itemsList = old('items', $combo->items);
                    @endphp

                    @foreach($itemsList as $idx => $item)
                        @php
                            $prodId = is_array($item) ? ($item['product_id'] ?? '') : $item->product_id;
                        ...
                        <div class="combo-item-row flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                            <div class="flex-1 flex items-center gap-2">
                                ...
                                <select name="items[{{ $idx }}][product_id]" class="product-select flex-1 px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:border-[#e8634a]" required>
                                    <option value="">-- Chọn sản phẩm --</option>
                                    @foreach($products as $prod)
                                        <option value="{{ $prod->product_id }}" data-price="{{ $prod->price }}" data-name="{{ mb_strtolower($prod->name) }}" {{ $prodId == $prod->product_id ? 'selected' : '' }}>
                            ...
                            <div class="flex items-center justify-between sm:justify-start gap-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <input type="number" name="items[{{ $idx }}][quantity]" value="{{ $qty }}" min="1" class="item-qty w-20 px-3 py-2 border rounded-lg text-sm text-center font-bold focus:outline-none focus:border-[#e8634a]" required>
                                </div>
