        ...
        <form action="{{ route('vouchers.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" placeholder="Ví dụ: CHILL50, GIAM20K" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a] uppercase">
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="discount_value" value="{{ old('discount_value') }}" placeholder="Ví dụ: 10 (cho 10%) hoặc 20000 (cho 20k)" required min="0" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Đơn hàng tối thiểu (đ) <span class="text-red-500">*</span></label>
                    <input type="number" name="min_order" value="{{ old('min_order', 0) }}" placeholder="Ví dụ: 50000" required min="0" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="start_date" value="{{ old('start_date') }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Ngày kết thúc</label>
                    <input type="datetime-local" name="end_date" value="{{ old('end_date') }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
            </div>
            ...
            <div class="pt-6 flex gap-4 justify-end">
                ...
                <button type="submit" class="px-6 py-3 bg-[#e8634a] text-white rounded-xl hover:bg-[#d5523b] font-medium shadow-sm">Tạo mã giảm giá</button>
            </div>
        </form>
    </div>
</div>
