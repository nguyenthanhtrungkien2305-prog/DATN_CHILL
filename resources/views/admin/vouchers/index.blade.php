...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <a href="{{ route('vouchers.create') }}" class="bg-[#e8634a] text-white px-6 py-2.5 rounded-xl hover:bg-[#d5523b] transition font-medium shadow-sm flex items-center justify-center gap-2">
...
<div class="bg-white p-4 rounded-t-2xl border-t border-x border-gray-100 flex items-center justify-between">
    <form action="{{ route('vouchers.index') }}" method="GET" class="w-full max-w-md flex gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm theo mã giảm giá..." class="flex-1 px-4 py-2 border rounded-xl focus:outline-none focus:border-[#e8634a] text-sm">
        ...
        @if(request('search'))
            <a href="{{ route('vouchers.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-xl hover:bg-gray-300 transition text-sm flex items-center">Reset</a>
        @endif
    </form>
</div>
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-gray-700 text-sm">
            ...
            <tr class="border-b hover:bg-gray-50 transition">
                <td class="p-4 text-gray-500 font-medium">#{{ $v->voucher_id }}</td>
                <td class="p-4 font-bold text-base"><span class="bg-orange-50 border border-orange-200 text-[#e8634a] px-2.5 py-1 rounded-md">{{ $v->code }}</span></td>
                <td class="p-4">
                    @if($v->discount_type === 'percent')
                        <span class="px-2.5 py-1 rounded-full text-xs bg-blue-50 text-blue-600 font-bold">Phần trăm (%)</span>
                    @else
                        <span class="px-2.5 py-1 rounded-full text-xs bg-green-50 text-green-600 font-bold">Số tiền cố định</span>
                    @endif
                </td>
                ...
                <td class="p-4">
                    <div class="flex justify-center gap-3">
                        <a href="{{ route('vouchers.edit', $v->voucher_id) }}" class="text-blue-600 hover:text-blue-800 font-medium px-3 py-1 bg-blue-50 rounded-lg">Sửa</a>
