...
@section('content')
    <header class="h-16 bg-white shadow-sm flex items-center justify-between px-8 shrink-0">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-xl font-semibold text-gray-800">Sửa Mã Giảm Giá</h2>
        </div>
    </header>
            ...
            <form action="{{ route('vouchers.update', $voucher->voucher_id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div id="points_required_container" class="{{ old('is_points_exchange', $voucher->is_points_exchange ?? 1) == 0 ? 'hidden' : '' }}">
                        ...
                        <input type="number" name="points_required" value="{{ old('points_required', $voucher->points_required ?? 10) }}" placeholder="Ví dụ: 10, 20, 50" min="0" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                    </div>
                </div>
