...
@section('content_class', 'flex-1 flex overflow-hidden p-0 h-[calc(100vh-4rem)]')
...
<div class="flex-1 flex w-full h-full overflow-hidden">
    ...
    <div class="w-80 bg-white border-r border-gray-200 flex flex-col h-full shrink-0">
        <div class="p-4 border-b border-gray-100 shrink-0">
            <input type="text" id="session-search" oninput="renderSessionSidebar()" placeholder="Tìm khách hàng..." class="w-full px-4 py-2 bg-gray-50 border border-transparent rounded-xl text-sm focus:outline-none focus:border-[#e8634a] focus:bg-white transition-all text-gray-800 placeholder-gray-400">
        </div>
    ...
    <div class="flex-1 flex flex-col bg-gray-50 h-full relative" id="chat-conversation-pane">
        ...
        <div id="chat-active-state" class="flex flex-col h-full w-full hidden">
            ...
            <form id="admin-chat-form" onsubmit="handleAdminSend(event)" class="p-4 bg-white border-t flex items-center gap-3 shrink-0">
                <input type="text" id="admin-chat-input" placeholder="Nhập câu trả lời của bạn..." autocomplete="off" class="flex-1 px-4 py-3 bg-gray-50 border border-transparent rounded-xl text-sm focus:outline-none focus:border-[#e8634a] focus:bg-white transition-all text-gray-800 placeholder-gray-400">
        ...
        if (sender === 'admin') {
            ...
            wrapper.innerHTML = `
                ...
                <div class="flex flex-col items-start max-w-[70%]">
                    <div class="bg-white text-gray-800 p-3.5 rounded-[18px] rounded-tl-none shadow-sm border border-gray-100 text-sm leading-relaxed">
                        ${formatMessageText(message)}
                    </div>
                    <span class="text-[9px] text-gray-400 mt-1">${time}</span>
                </div>
