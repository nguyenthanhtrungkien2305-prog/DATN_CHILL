        ...
        <form action="{{ route('products.update', $product->product_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a]">
                </div>
                    ...
                    <div id="variants-container" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        @if(isset($variants) && count($variants) > 0)
                            @foreach($variants as $variant)
                                <div class="relative bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div class="flex gap-4 items-start bg-gray-50 p-4 rounded-xl border border-gray-200">
                        ...
                        <div class="w-full space-y-3">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                                ...
                                <input type="text" name="image_url" value="{{ old('image_url', $product->image_url) }}" class="w-full px-4 py-2 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a] text-sm" placeholder="https://example.com/image.jpg">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-[#e8634a]">{{ old('description', $product->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="pt-6 border-t flex gap-4 justify-end">
                ...
                <button type="submit" class="px-6 py-3 bg-[#e8634a] text-white rounded-xl hover:bg-[#d5523b] transition font-medium shadow-sm">Lưu thay đổi</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function addSizeRow() {
        ...
        div.className = 'relative bg-white p-3 rounded-lg border border-[#e8634a] shadow-md flex flex-col gap-2';

        div.innerHTML = `
            <button type="button" onclick="this.parentElement.remove()" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold hover:bg-red-600 shadow z-10" title="Xóa">✕</button>
